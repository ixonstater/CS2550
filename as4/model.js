...
GameModel.prototype.insertRandom = function (mergedGrid) {
    let possibleIndicies = this.getEmptyIndicies(mergedGrid)

    let index = possibleIndicies[Math.floor(Math.random() * possibleIndicies.length)]
    let val = Math.random() < 0.5 ? 2 : 4
    this.selectedCell = [index, val]
    mergedGrid[index[0]][index[1]] = '' + val

    return mergedGrid
}

GameModel.prototype.getEmptyIndicies = function (mergedGrid) {
    let indicies = []
    for (let x = 0; x < 4; x++) {
        for (let y = 0; y < 4; y++) {
            if (mergedGrid[x][y] == '0') {
